...
export function computeRecoupe(project, budgetLines, royaltyRows, eurRate) {
  ...
  // ── 5. Phase 0 : Recoupes artiste & fabrication (parallèles) ─
  // Sur les royalties qui arrivent réellement à Avlanche

  // Théorique de l'artiste
  const artistTheoretical = avlancheRevenue * artistRate / 100
  const artistAdvanceRecouped = Math.min(artistTheoretical, artistAdvance)
  const artistAdvanceRemaining = Math.max(0, artistAdvance - artistTheoretical)
  const artistAdvancePct = artistAdvance > 0
    ? Math.min((artistTheoretical / artistAdvance) * 100, 100)
    : 100
  const artistAdvanceDone = artistTheoretical >= artistAdvance
  // Cash perçu par l'artiste = ce qui dépasse son avance
  const artistCash = Math.max(0, artistTheoretical - artistAdvance)

  // Théorique du label (= 100 - artistRate% des revenus avlanche)
  const labelTheoretical = avlancheRevenue - artistTheoretical
  const fabricationRecouped = Math.min(labelTheoretical, fabricationCost)
  const fabricationRemaining = Math.max(0, fabricationCost - labelTheoretical)
  const fabricationPct = fabricationCost > 0
    ? Math.min((labelTheoretical / fabricationCost) * 100, 100)
    : 100
  const fabricationDone = labelTheoretical >= fabricationCost

  // ── 6. Phase 1 : Bénéfices ─────────────────────────────────
  // Une fois l'avance artiste ET la fabrication recoupées
  const profitsAvailable = artistAdvanceDone && fabricationDone

  // Surplus du label après remboursement fabrication
  const labelProfit = Math.max(0, labelTheoretical - fabricationCost)
