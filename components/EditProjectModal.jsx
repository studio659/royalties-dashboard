...
export default function EditProjectModal({ serie, onClose, onSuccess }) {
  ...
  const [artistAdvance, setArtistAdvance] = useState(serie.artist_advance || 0)
  const [coprodName, setCoprodName] = useState(serie.coprod_name || '')
  const [coprodRate, setCoprodRate] = useState(serie.coprod_rate || 0)
  const [labelRate, setLabelRate] = useState(serie.label_rate || 100)
  const [notes, setNotes] = useState(serie.notes || '')

  // Distrib fields
  const [distribName, setDistribName] = useState(serie.distrib_name || '')
  const [distribAdvance, setDistribAdvance] = useState(serie.distrib_advance || '')
  const [distribRate, setDistribRate] = useState(serie.distrib_rate || '')

  useEffect(() => { fetchSingles() }, [])
  ...
  async function saveContract() {
    ...
    await supabase.from('series').update({
      ...
      distrib_name: distribName || null,
      distrib_advance: distribAdvance ? parseFloat(distribAdvance) : null,
      distrib_rate: distribRate ? parseFloat(distribRate) : null,
      notes,
    }).eq('id', serie.id)
    setSaving(false)
    onSuccess()
  ...
  async function addBudgetLine(singleId, label, amount) {
    ...
    await supabase.from('budget_lines').insert({ single_id: singleId, label, amount_eur: parseFloat(amount) || 0 })
    const { data } = await supabase.from('budget_lines').select('amount_eur').eq('single_id', singleId)
    const total = (data || []).reduce((s, l) => s + l.amount_eur, 0)
    await supabase.from('singles').update({ budget_eur: total }).eq('id', singleId)
    fetchSingles()
  ...
  const isWarner = serie.distrib_advance != null
    ...
    <div className="ov" onClick={!saving ? onClose : undefined}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="mh">
          <div>
            ...
            <div className="mh-tabs">
              ...
              <button className={`mt ${tab === 'distrib' ? 'act' : ''}`} onClick={() => setTab('distrib')}>Distribution</button>
        ...
        <div className="mb">
          {tab === 'contract' && (
            <div>
              <div className="hbox">Les modifications s'appliquent aux calculs. Les royalties déjà générées ne changent pas.</div>

              <div className="field">
                <label>Avance artiste (€)</label>
                <input type="number" value={artistAdvance} onChange={e => setArtistAdvance(e.target.value)} min="0" />
              </div>
          ...
          {tab === 'distrib' && (
            <div>
              <div className="hbox">
                Schéma de distribution. Si avance distrib renseignée, elle est recoupée avant qu'Avlanche ne perçoive les royalties.
              </div>

              <div className="field">
                <label>Nom du distributeur (laisser vide pour agrégateur DistroKid/TuneCore)</label>
                <input value={distribName} onChange={e => setDistribName(e.target.value)} placeholder="ex: Warner" />
              </div>

              <div className="row2">
                <div className="field">
                  <label>Avance distrib (€)</label>
                  <input type="number" value={distribAdvance} onChange={e => setDistribAdvance(e.target.value)} min="0" placeholder="ex: 110000" />
                </div>
                <div className="field">
                  <label>Taux distrib (%)</label>
                  <input type="number" value={distribRate} onChange={e => setDistribRate(e.target.value)} min="0" max="100" step="0.5" placeholder="ex: 25" />
                </div>
              </div>

              {isWarner && (
                <div className="hbox" style={{ marginTop: 12, color: '#f59e0b' }}>
                  ⚠️ Modifier l'avance distrib affecte tous les calculs de recoupe de ce projet.
                </div>
              )}
            </div>
          )}
            ...
            <div>
              {singles.map(single => (
                <div key={single.id} className="single-block">
                  ...
                  <div className="sb-budget">Fabrication : {fmtEur(single.budget_eur || 0)}</div>
      ...
      <style jsx>{`
        ...
        .hbox{background:#0f0f0f;border:1px solid #1e1e1e;border-radius:6px;padding:10px 12px;font-size:11px;color:#666;margin-bottom:14px;line-height:1.5}
